...
In general, this exercise showed both the power and limitations of simple classifiers and traditional computer vision techniques. I also found the heat mapping approach to be a great tool for filtering false positives when search areas overlap.  I would be interested to see how much improvement I could get in initial detection by using a deep learning approach instead of a simple classifier.  
...
#### Detection accuracy
Although I had an extremely high test accuracy on the training data, I also noticed a fairly high instance of false positives in the video.  Thus, I assume my classifier was not generalizing well.  I could have tried additional HOG parameter tuning, SVM parameter tuning, or additional features (spatial, histogram, etc) to improve this.  Also, as noted in the tips for the project, the input training data included some time series data, which may have lead to overfitting, depending on the random split of training/test data.  I could have manually separated the data in a better way and/or included more labeled images from other sources.  
